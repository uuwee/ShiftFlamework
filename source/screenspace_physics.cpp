...
void ScreenSpacePhysics::update() {
  for (auto body_wptr : body_list) {
    if (std::shared_ptr<RigidBody> body = body_wptr.lock()) {
      ...
      for (const auto circle : body_list) {
        ...
        if (std::shared_ptr<RigidBody> other_body = circle.lock()) {
          ...
          if (Math::Length(pos_diff) < (body->radius + other_body->radius)) {
            ...
            body->velocity += -2.0 * Math::Dot(body->velocity, normal) * body->velocity * body->restitution * 0.1;
            other_body->velocity += +2.0 * Math::Dot(body->velocity, normal) *
                                    other_body->velocity *
                                    other_body->restitution * 0.1;

            body->entity->get_component<ScreenSpaceTransform>()->position +=
                -Math::GetNormalized(pos_diff) *
                (body->radius + other_body->radius - Math::Length(pos_diff)) * 0.5 * 1.1;
            other_body->entity->get_component<ScreenSpaceTransform>()->position +=
                Math::GetNormalized(pos_diff) *
                (body->radius + other_body->radius - Math::Length(pos_diff)) * 0.5 * 1.1;
      ...
      if (transform->position.internal_data[1] != transform->position.internal_data[1]) {
        body->velocity += body->gravity;
      }

      body->velocity = (transform->position - old_position) * body->deceleration;
    }
  }
}