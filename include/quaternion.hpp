...
inline Quaternionf get_normalized(const Quaternionf& p) {
  ...
  auto normalized =
      Quaternionf({p.x / dir_len, p.y / dir_len, p.z / dir_len, 1.0});
  return normalized;
}

inline Quaternionf operator*(const Quaternionf& q, const Quaternionf& p) {
  return Quaternionf({q.w * p.x - q.z * p.y + q.y * p.z + q.x * p.w,
...
inline Quaternionf get_inverse(const Quaternionf& p) {
  float dir_len = length(Vector3f({p.x, p.y, p.z}));
  return Quaternionf(
      {-p.x / dir_len, -p.y / dir_len, -p.z / dir_len, p.w / dir_len});
}
...
inline Quaternionf angleaxis(float angle, Vector3f axis) {
  ...
  return Quaternionf({v.x * sin(angle / 2), v.y * sin(angle / 2),
                      v.z * sin(angle / 2), cos(angle / 2)});
}
...
inline Matrix<float, 4, 4> transMatrix(Vector3f position_xyz,
